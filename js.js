...
function getData() {
  ...
  $.ajax({
    ...
    success: function (response, status, request) {
      ...
      // Ensure there are at least 9 rows in table type D
      const numEmptyRows_d = Math.max(9 - res_d.length, 0);
      ...
      // Ensure there are at least 9 rows in table type B
      const numEmptyRows = Math.max(9 - res_b.length, 0);
